...
// Mutation function to be passed into Creature's brain
function mutate(x) {
  if (random(1) < 0.1) {
...
// This is the class for each Creature
class Creature {
  ...
  clone(prob) {
    ...
    if (r < prob) {
      ...
      return new Creature(this.brain);
    }
  ...
  display() {
    ...
    if (debug.checked()) {
      ...
      // Display score and neural net next to each vehicle
      noStroke();
      fill(255, 200);
      text(int(this.score), 10, 0);
      if(dist(mouseX,mouseY,this.position.x,this.position.y)<35){
        this.istouched++;
      }
      if(this.istouched>0){
        this.brain.disp(-96,-44);
      }
