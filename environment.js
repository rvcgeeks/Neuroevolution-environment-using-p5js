...
function setup() {
  ...
  let canvas = createCanvas(950, 1300);
...
function draw() {
  ...
  for (let n = 0; n < cycles; n++) {
    ...
    //get the best life from iteration...
    ...
    if (population.length < 20) {
      for (let v of population) {
        ...
        let newCreature = v.clone(0.1 * v.score / record);
        // If there is a child
        if (newCreature != null) {
  ...
  //display the best record of iteration...
  document.getElementById("bestone").innerHTML = bestrecord;
  // Draw all the food
